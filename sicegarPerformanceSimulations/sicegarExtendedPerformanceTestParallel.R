...
parallel_Result_SM <- foreach(counter01 = 1 : nFirstLoopSM) %dopar%
{
  ...
  ###*****************************
  # Generate data frame
  sigmoidalOutputSub <- data.frame(runCount=double(), noiseLevel=double(), distinctRunNo=double(), decision = character(),
...
parallel_Result_DSM <- foreach(counter01 = 1:nFirstLoopDSM) %dopar%
{
  ###*****************************
  # Generate data frame
  doubleSigmoidalOutputSub <- data.frame(runCount=double(), noiseLevel=double(), distinctRunNo=double(), decision = character(),
                                         maximumSM=double(), midpoint=double(), slopeParam=double(),
                                         maximumDSM=double(), midPoint1Param=double(),
                                         slope1Param=double(), slope2Param=double(),
                                         midPointDistanceParam=double(), finalAsymptoteIntensityRatio=double())
