...
distinct_model_parameters <- 10  # Used value in supplementary figure is "10"
...
# Sigmoidal
sigmoidal_parameters <- function(true_model)
{
  if(true_model == "SM")
  {
...
double_sigmoidal_parameters <- function(true_model)
{
  if(true_model == "DSM")
  {
...
df %>%
  ...
  dplyr::do(sm_param = sigmoidal_parameters(.$true_model)) -> df2_sm

df2_sm %>%
  dplyr::group_by(true_model, noise_type, time_sampling,
                  noise_parameter, distinct_model_parameters, distinct_runs, run_no) %>%
  dplyr::do(.,as.data.frame(t(unlist((.$sm_param))))) -> df3_sm

df %>%
  group_by(true_model, noise_type, time_sampling,
           noise_parameter, distinct_model_parameters, distinct_runs, run_no) %>%
  dplyr::do(dsm_param = double_sigmoidal_parameters(.$true_model)) -> df2_dsm

df2_dsm %>%
  dplyr::group_by(true_model, noise_type, time_sampling,
                  noise_parameter, distinct_model_parameters, distinct_runs, run_no) %>%
  dplyr::do(.,as.data.frame(t(unlist((.$dsm_param))))) -> df3_dsm

dplyr::left_join(df2_sm, df2_dsm) -> df2
dplyr::left_join(df3_sm, df3_dsm) -> df3
...
df3 %>%
  dplyr::mutate(n_samples = n_samples, t_min = t_min, t_max = t_max) -> df3

# Save the df
write.csv(x = df3, file = "distinct_runs_supplementary_fig.csv", row.names = F)

# Remove unnecessary data frames
df <- df2
remove(df2_dsm, df2_sm, df3_dsm, df3_sm, df3, df2)
###*****************************
...
df %>%
  ...
  dplyr::do(p_model_parameters = main_function(condition_vector = ., error_mode = FALSE)) -> df_prediction
