...
{setwd(paste0("/Users/umut/GitHub/sicegar_mathematica_appx/sicegarPerformanceSimulations/"))} # mac computer
###*****************************
...
# The parameters used
noiseParameterValue <- seq(from= 0, to = 1.5, length.out =11)
distinctRuns <- 3
distinctParameters <- 50


#for (counter01 in 1:distinctParameters)
parallel_Result_SM <- foreach(counter01=1:distinctParameters) %dopar%
{
  sigmoidalOutputSub <- data.frame(noiseLevel=double(), distinctRunNo=double(), decision = character(),
  ...
  maximumSMValue <- runif(1, 0.3, 20)
  slopeParamValue <- runif(1, 0.001, 40)
  midpointValue <- runif(1, 3, 27)
  ...
  for (counter02 in 1:length(noiseParameterValue))
  {
    for (counter03 in 1:distinctRuns)
    {
      ...
      intensity_noise <- stats::runif(n = length(time), min = -0.5, max = 0.5) * noiseParameterValue[counter02] * maximumSMValue
      ...
      sigmoidalOutputSub = dplyr::bind_rows(sigmoidalOutputSub, tempOutput)
    }
  }
  list(sigmoidalOutput=sigmoidalOutputSub)
}
###*****************************
...
{
  doubleSigmoidalOutputSub <- data.frame(noiseLevel=double(), distinctRunNo=double(), decision = character(),
                                         maximumSM=double(), midpoint=double(), slopeParam=double(),
                                         maximumDSM=double(), midPoint1Param=double(), 
                                         slope1Param=double(), slope2Param=double(), 
                                         midPointDistanceParam=double(), finalAsymptoteIntensityRatio=double())
  ...
  for (counter02 in 1:length(noiseParameterValue))
  {
    for (counter03 in 1:distinctRuns)
    {
      ...
      doubleSigmoidalOutputSub = dplyr::bind_rows(doubleSigmoidalOutputSub,tempOutput)
    }
  }
  list(doubleSigmoidalOutput = doubleSigmoidalOutputSub)
  
}

parallel_Result_SM %>% purrr::map_df(bind_rows)->sigmoidalOutput
parallel_Result_DSM %>% purrr::map_df(bind_rows)->doubleSigmoidalOutput

sigmoidalOutput$realInput <- "sigmoidal"
doubleSigmoidalOutput$realInput <- "double_sigmoidal"

write.csv(x = sigmoidalOutput, file = "sigmoidalPerformanceTestResultsPar.csv")
write.csv(x = doubleSigmoidalOutput, file = "doubleSigmoidalPerformanceTestResultsPar.csv")
###*****************************