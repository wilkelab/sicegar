...
sigmoidalModelAugmented <- sicegar::parameterCalculation(sigmoidalModel)
```

```{r echo=FALSE, warning=FALSE, results='hide', message=FALSE}
doubleSigmoidalModelAugmented <- sicegar::parameterCalculation(doubleSigmoidalModel)
```
...
The following parameters are calculated by `sicegar:parameterCalculation()` for the sigmoidal model.
